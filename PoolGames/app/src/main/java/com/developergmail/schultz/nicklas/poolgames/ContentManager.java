...
public class ContentManager extends Application {
    ...
    private ArrayList<IGame> getDummyGames() {
        ...
        IGame pool = new Cribbage("Cribbage", new Rule(
                ...
                "The penalty for all fouls is the ending of the player's inning; no points are lost, and the incoming player has the option of shooting from position or taking cue ball in hand from the kitchen (behind the table's head string). Pocketing the 15 ball when it is not the last ball on the table is not a foul. Instead it is immediately spotted and play continues without penalty\n\n" +
                "When players pocket more than one ball on a single stroke at any time, they may shoot at any companion balls, but must pocket each in succession in any order. If incidental balls are pocketed on the same stroke that a cribbage is completed, they add to the succession of cribbages the player is \"on\". When a player fouls by failing to pocket an unpaired cribbage while on a succession of unpaired balls, only unpaired balls are spotted; the prior successful cribbages count toward the score\n\n" +
